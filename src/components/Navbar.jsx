    ...
    <header className="sticky top-0 z-50 backdrop-blur bg-white/70 dark:bg-slate-950/60 border-b border-slate-200/60 dark:border-slate-800/60">
      <div className="container flex items-center justify-between py-3">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {links.map(l => (
            <Link key={l.href} to={l.href} className="text-sm hover:text-sky-600 dark:hover:text-sky-400 transition-colors">{l.label}</Link>
          ))}
          <Link to="/contact" className="inline-flex items-center text-sm font-medium px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-soft hover:opacity-90">Get started</Link>
          <ThemeToggle/>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur">
          <div className="container py-3 flex flex-col gap-3">
            {links.map(l => (
              <Link key={l.href} to={l.href} onClick={() => setOpen(false)} className="text-sm py-1">{l.label}</Link>
            ))}
            <Link to="/contact" onClick={() => setOpen(false)} className="inline-flex items-center justify-center text-sm font-medium px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900">Get started</Link>
          </div>
        </div>
      )}
    </header>
