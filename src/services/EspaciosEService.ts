...
const getStaticEspaciosE = (): Espacio[] => {
  const serie1 = new Serie(
    ...
    [
      new Taller(
        1,
        "Taller 1. La estructura básica del ser. El asiento de lo que el cuerpo calla.",
        ""
      ),
      new Taller(
        2,
        "Taller 2. Expresión de la estructura básica. La identidad y los espejos.",
        ""
      ),
      new Taller(
        3,
        "Taller 3. Mecanismo de expresión de la estructura básica. Parte 1: Las emociones, sentimientos, sensaciones. Las VAC.",
        ""
      ),
      new Taller(
        4,
        "Taller 4. Mecanismo de expresión de la estructura básica. Parte 2: Los pensamientos, las creencias y los valores.",
        ""
      ),
      new Taller(
        5,
        "Taller 5. Mecanismo de expresión de la estructura básica. Parte 3: El lenguaje comunicacional del cuerpo.",
        ""
      ),
      new Taller(6, "Taller 6. La expresión de la denuncia.", ""),
      new Taller(
        7,
        "Taller 7. La autogestión de las memorias silenciadas.",
        ""
      ),
    ]
  );
  ...
  nivel2EcoSer.series.push(
    new Serie(
      ...
      [
        new Taller(
          1,
          "Taller I. La transformación interna y externa parte 1.",
          ""
        ),
        new Taller(
          2,
          "Taller II. La transformación interna y externa parte 2.",
          ""
        ),
      ]
    )
...
export const getEspaciosE = async (): Promise<Espacio[]> => {
  // Aquí podrías implementar la lógica para obtener los datos desde una API o base de datos
  // Por ahora, simplemente retornamos los datos estáticos
  return getStaticEspaciosE().map((espacio) => {
    return new Espacio(espacio.id, espacio.nombre, espacio.descripcion, []);
  });
};

export const getEspacioEById = async (id: number): Promise<Espacio | null> => {
  const espacios = getStaticEspaciosE();
  const espacio = espacios.find((e) => e.id === id);
  return espacio ? espacio : null;
};
